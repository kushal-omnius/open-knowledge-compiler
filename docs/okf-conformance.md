...
**Resolved, formerly an architectural note:** `wiki/emitter.py` (what shape output takes) and `wiki/okf_conformance.py` (what shape is required) used to encode the same structural facts — reserved-filename frontmatter restrictions, required concept-page fields — independently, with nothing structurally preventing the two from drifting apart over time. [ADR-014](decisions/ADR-014-shared-okf-rules-file.md) (Accepted, implemented 2026-08-26) unified both behind one shared declarative rules file (`wiki/okf_rules.py`) — `okf_conformance.py`'s checker is now a generic interpreter over it, and `emitter.py` self-checks against the same object at emission time, failing loudly on drift instead of only via a separate downstream `kc validate-okf` run. A future spec-version bump is now a data diff to `okf_rules.py`, not two independent code edits kept in sync by hand.
