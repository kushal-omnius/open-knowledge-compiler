...
- **Progress:** `[jira] i/n <scope>: R/N keys resolved (missing: ...)` is emitted to stderr whenever a compile item extracts at least one issue key — `<scope>` is `PR #<number>`, `commit <short-sha>`, or `full compile` (the `_file_jira_all_keys` enrichment fallback). Previously silent by design: a key that fails to resolve is data, not an outage (same contract as a genuinely nonexistent key), but that meant success and silent under-resolution looked identical from the terminal — this line makes both visible without changing the underlying "absence is data" contract.
...
- **Additive (ADR-023):** `_state_models` mints `state_model` entities from `state_transition_observed` facts (Python analyzer, ADR-006-style deterministic extraction — states plus structurally-inferred transitions, never fabricating an edge across a branch construct's outcomes) grouped by (owning Component, field), with a `models` relationship to the owning Component; a fact whose module doesn't resolve to a compiled Component this compile is silently skipped, same class as `_mutation_scores`'s unresolved-module precedent.
- **Additive (ADR-020):** `_escaped_defects` merges `escaped_defect_observed` facts (Extract-stage classification of a merged PR as a bug fix — title/body regex or a linked Jira issue typed `Bug`, PR-triggered compiles only) into the matching Component's payload as a cumulative trust score. "Covered as of the preceding compile" is read from `current.relationships`' `covers` edges (the only durable coverage signal this IR has — the `kc-covers:` declared-coverage header is never persisted); prior cumulative counts are read from `current`'s prior Component payload and incremented, the same carry-forward pattern `_mutation_scores` uses for its own fields.
