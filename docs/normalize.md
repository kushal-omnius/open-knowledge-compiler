...
- **Jira Story → Pull Request (`motivates`)** *(additive clarification)*: minted from `jira_observed.linked_pr` (set at Collect time from the originating PR's title/body issue-key match — architecture.md §6's forge-linkage rule, extended to Jira) — a direct field lookup, not a path-resolution cascade like the component relationships above. Jira Story → Feature `motivates` linkage (ir.md §3.3) is not attempted: it would need an LLM-derived signal connecting an issue to a feature, which no current fact type provides.
