...
- **Status:** Accepted
- **Summary:** Compilation is triggered by CI invoking `kc compile --pr N` on merge, made exactly-once and self-healing by mandatory reconcile-first processing and per-repo advisory locks; the serve process never compiles. Addendum (2026-08-09): reconcile now runs two passes — PR-based (`list_merged_prs`) then commit-fill (`list_commits`) covering direct pushes and repos without a PR workflow; watermark unified to `max(COALESCE(commit_timestamp, merged_at))`; `compile_runs.scope` gains `'commit'` (migration 0006).
...
- **Status:** Accepted — the spec-version mechanism (item 2) is implemented; verified against code 2026-08-26, status corrected without any new code
- **Summary:** Open-sourcing Knowledge Compiler as `open-knowledge-compiler` positions it as a reference OKF producer; since the OKF spec is external and independently versioned (discovered mid-review: the authoritative spec had already moved to v0.2, exposing real conformance bugs against KC's v0.1-era emitter), track `OKF_SPEC_VERSION` explicitly (extending ir.md §5's `fact_vocabulary_version`/`knowledge_model_version` pattern) and handle all future spec migrations as Emit-stage-only re-renders against durable Knowledge IR — never a data migration. Database-less compilation, raised in the same discussion, is explicitly parked out of scope (`BRAINSTORM-db-less-compile-mode.md`). Item 1, the open-source release itself (licensing, scrubbing internal references, community docs), remains genuinely unmade and was never this ADR's scope.
...
- **Summary:** `wiki/emitter.py` (what shape output takes) and `wiki/okf_conformance.py` (what shape is required) used to encode the same OKF structural facts — reserved-filename frontmatter restrictions, required concept-page fields — independently, with nothing preventing the two from silently drifting apart. A new shared rules module (`wiki/okf_rules.py` — a Python dataclass, not the JSON file this ADR's Open Questions considered, to avoid packaging risk) is now read by both sides: `okf_conformance.py`'s `check_bundle()` is a generic interpreter over it (verified data-driven, not filename-hardcoded, by tests passing a synthetic rules object), and three points in `emitter.py` (`_frontmatter`, `_render_index`, `_render_log`) self-check against the same object, raising loudly at emission time rather than only being caught by a separate downstream `kc validate-okf` run — verified live, not decorative, by tests that monkeypatch the shared rules to a drifted version and confirm each self-check actually fires. Explicitly scoped to structural rules only — content-value computation and future type-specific concept schemas (e.g. `Attested Computation`) stay out of scope until actually needed.
...
- **Status:** Accepted — implemented 2026-08-26, PR-triggered compiles only
- **Summary:** A forward-looking, PR-triggered correlation between bug-fix PRs (title/body regex or a linked Jira issue typed "Bug") and whether the Component they touched already had compiled `Test Coverage` (the `covers` relationship — an implementation-time resolution, since the `kc-covers:` declared-coverage header is never durably compiled, only computed statelessly by `kc validate-test`) — a cheaper alternative to the eval brainstorm's rejected full historical-bug-replay harness (Option E), producing a cumulative per-Component trust score (`round(1 - covered_fix_count/fix_count, 4)`, withheld below 3 observed fixes) that measures outcomes rather than input proxies. Surfaced via `coverage_for`/`test_plan`'s new `low_escaped_defect_trust`, informational only. Zero-value until real fix history accumulates, by design — this is a longitudinal signal, so it was implemented and deployed now precisely to start that clock, not deferred to when it would already be useful.
...
### [ADR-022 — External-Key Inline Annotations (`kc:external-key:`)](ADR-022-external-key-annotations.md)

- **Status:** Accepted
- **Summary:** Adds a `# kc:external-key: <key>` source-code annotation that pins LLM-derived entities (features, business rules, risks) to a stable external key, bypassing the anchor-overlap and name-similarity cascade steps. Parsed deterministically from source files before the LLM call; injected into the fact payload post-LLM-validation so the LLM cache key is unaffected and the LLM never assigns identity. Fixes two failure modes observed in KC's self-compile: LLM stochasticity on cache miss (entity not re-extracted after a file change) and anchor drift (LLM re-anchors a concept to a different call site across re-extractions, breaking Rule 2). V1 supports Python `def` and `class` annotations only; method-level annotations and multi-rule class annotations are deferred.
- **Dependencies:** ADR-004 (Rule 1 external-key cascade step this annotation feeds), ADR-008 (LLM cache-key contract — unchanged)
- **Depended on by:** —
- **Related documents:** `knowledge_compiler/extractors/annotation_parser.py`, `knowledge_compiler/extractors/llm_extractor.py`, `knowledge_compiler/compiler/run.py` (`_extract_semantic`)

### [ADR-023 — State Model Entity for Behavioral Contract Knowledge](ADR-023-state-model-entity.md)

- **Status:** Accepted — implemented 2026-08-26, Python analyzer only (V1 scope, TypeScript/JavaScript deferred)
- **Summary:** Adds `state_model` as a new, deterministic-identity entity type (natural key: owning Component path + field name) representing a resource's own lifecycle — states plus structurally-inferred transitions — closing the gap an external review identified: nothing in the compiled Knowledge Model previously represented that a resource is a state machine, only static invariants (Business Rule) or human-declared cross-resource traversals (User Journey). Extraction (Python analyzer only) scans `<x>.status`/`<x>.state = "literal"` assignments; branch constructs (if/elif/else, try/except) never leak state across branches, resetting to unknown after a branch merge rather than fabricating a transition between two mutually-exclusive outcomes — verified against real dogfood code (frida's `ConversionJob` job-status machine and `admin/services.py`'s draft/released/active/deprecated/archived bundle lifecycle) before being trusted. `test_plan` gains a `transition_gap` target_kind surfacing the known transition set (each edge labeled `confidence: "structural"`) for review, not a per-edge covered/uncovered verdict. Cross-process transitions (the hardest cases in the motivating payment-API example — duplicate/late/out-of-order webhook, retry-after-timeout) are explicitly out of scope for this deterministic-only slice, same discipline as ADR-012's and ADR-017's precedent of shipping less than the full design first.
- **Dependencies:** ADR-006 (deterministic-first extraction), ADR-009 (two-layer IR boundary), ADR-004 (deterministic identity class, same as Component/API)
- **Depended on by:** —
- **Related documents:** `BRAINSTORM-state-model.md` (full option exploration and spike), ADR-012 (VerificationRequirement — the guarded-transition connection noted in Open Questions), ADR-017 (UserJourney — closest structural precedent)
...
graph TD
    ...
    ADR012 --> ADR008
    ADR013["ADR-013 OKF Conformance + Migration ✓"] --> ADR001
    ADR013 --> ADR003
    ADR013 --> ADR004
    ADR013 --> ADR009
    ADR013 --> ADR010
    ADR014["ADR-014 Shared OKF Rules File ✓"] --> ADR013
    ADR014 --> ADR007
    ...
    ADR019 --> ADR007
    ADR020["ADR-020 Escaped-Defect Trust Score ✓"] --> ADR002
    ADR020 --> ADR012
    ADR020 --> ADR018
    ADR021["ADR-021 Jira Gateway Source Abstraction ✓"] --> ADR002
    ADR021 --> ADR007
    ADR022["ADR-022 External-Key Annotations ✓"] --> ADR004
    ADR022 --> ADR008
    ADR023["ADR-023 State Model Entity ✓ (Python only)"] --> ADR006
    ADR023 --> ADR009
    ADR023 --> ADR004
```

ADR-001 through ADR-010 were Accepted as of the 2026-07-18 v1.0 freeze; ADR-011 was added 2026-07-20, recording a genuinely new decision reached during dogfood; ADR-012 was added 2026-07-29, recording the VerificationRequirement deferral decision reached after milestone-3 test-generation spikes. ADR-013 was proposed 2026-08-05, recording the open-source release plan and the OKF v0.1→v0.2 spec-drift discovery. ADR-014 was proposed 2026-08-06, recording a design to unify the wiki emitter and OKF validator behind one shared rules file — unbuilt at the time (see below for its 2026-08-26 implementation). ADR-015 and ADR-016 were proposed 2026-08-06, recording designs for JavaScript and Java language-analyzer support respectively; ADR-015 was implemented the same day and its status moved to Accepted, while ADR-016 (Java) remains Proposed and unimplemented. ADR-017 through ADR-020 were proposed 2026-08-08, recording a QA-agent-test-grounding backlog surfaced by thinking through what a QA agent needs beyond declared-coverage percentages; ADR-017 (UserJourney) and ADR-018 (stale-test detection) were implemented the same day (ADR-017 with its extraction scope deliberately reduced from the proposed hybrid design to a deterministic-only `kc.toml`-declared slice — see its Status section) and their statuses moved to Accepted, while ADR-019 (test flakiness) and ADR-020 (escaped-defect trust score) remain Proposed and unimplemented, explicitly out of scope for that PR. ADR-021 was added 2026-08-09, recording the Jira collector's second gateway backend (a pre-fetched file cache, for agent-driven compiles with Jira access but no configured API token) surfaced by working through how an agent could actually get Jira data into KC without a token; implemented the same day, status Accepted. The ADR-002 addendum (also 2026-08-09) extended reconcile with a commit-fill pass (`ForgeGateway.list_commits`) covering direct pushes, squash commits, and repos without a PR workflow, and added the Jira→Feature enrichment path (`jira_feature_link_observed` fact type, `_jira_feature_enrichment_facts` in Extract, `motivates→Feature` edges in Normalize) plus wiki emitter backlinks ("Motivated by Jira" section on feature pages). ADR-022 was added 2026-08-10, recording `# kc:external-key:` inline annotations that pin LLM-derived entities to a stable external key, making them immune to anchor drift and LLM stochasticity on cache miss; motivated by false wiki-page deletions observed in KC's self-compile when `schema.py` changed; implemented the same day, status Accepted. ADR-023 was added 2026-08-26, recording the `state_model` entity — a resource's own states/transitions, closing the "Behavioral Contract" gap named in CLAUDE.md's north star and surfaced by an external review of the compiled Knowledge Model; implemented the same day for the Python analyzer only (TypeScript/JavaScript deferred pending demand, same discipline as ADR-016), status Accepted. ADR-020 was implemented the same day (2026-08-26), closing the "Test Trust Score" north-star capability's outcome-based leg — deliberately deployed now, before real fix history exists, since the signal is longitudinal and needs time in production to become useful; ADR-019 (test flakiness) was explicitly left unimplemented in this pass, by direction, so ADR-020's dependency on it is a design-precedent citation only, not a build blocker. ADR-014 was also implemented the same day, closing the emitter/validator drift risk it named with a shared rules module (`wiki/okf_rules.py`) both sides now consult and self-check against. ADR-013's status was corrected from Proposed to Accepted the same day, without any new code: a direct code audit confirmed its spec-version-tracking mechanism (item 2 — `OKF_SPEC_VERSION`, `compile_runs.okf_spec_version`, `index.md`'s `okf_version` field, `kc compile --emit-only`) was already fully implemented, most likely landed alongside earlier OKF v0.2 conformance fixes without the ADR's own status line being updated at the time; item 1 (the open-source release itself) remains genuinely unmade and was never this ADR's scope. All Accepted ADRs are immutable per the process in [README.md](README.md) — changes require a superseding ADR; ADR-016 and ADR-019 remain Proposed pending review, and ADR-016 (Java analyzer) remains explicitly excluded from this backlog pass by direction.
