...
**Decisive uncertainty:** whether `mutmut` (or an equivalent) is actually the right mutation-testing tool for this codebase in mid-2026 — this session did not verify that with a fresh check, and it's an assumption, not a researched fact. If mutation testing turns out impractical at this repo's scale/complexity (long test run times, flaky live-dependency tests in frida's suite), Option C degrades toward Option B (declared-coverage only, execution signal dropped) rather than failing outright — a safe fallback, not a cliff.

**Cheap spike to resolve it (≤ half a day):** Pick one already-compiled component with existing test coverage (e.g. `component/backend-claims-routes`, which we already know has 13 associated APIs and existing tests in frida's own `backend/tests/`), run a mutation-testing tool against just that file, and see whether it completes in reasonable time and produces mutants that a human agrees are meaningful. If yes, Option C's execution-based half is validated cheaply before building the full harness.
...
1. Run the mutation-testing spike above (≤ half a day) to de-risk Option C's decisive uncertainty before committing further design effort.
...
4. Only after 1–3: begin the actual milestone-3 test-generation mechanism design, now with a defined success metric to build toward.
